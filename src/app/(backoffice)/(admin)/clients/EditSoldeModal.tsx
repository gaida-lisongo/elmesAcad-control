...
import { updateAccountSolde } from "@/lib/actions/clients-management-actions";

interface EditSoldeModalProps {
  accountId: string;
  currentSolde: number;
  onClose: () => void;
  onSuccess: () => void;
...
export default function EditSoldeModal({
  accountId,
  currentSolde,
  onClose,
  onSuccess,
}: EditSoldeModalProps) {
  const [solde, setSolde] = useState(currentSolde);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await updateAccountSolde(accountId, solde);

      if (result.success) {
        setMessageType("success");
        setMessage("Solde mis à jour avec succès!");
        setTimeout(() => {
          onSuccess();
          onClose();
        }, 1500);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <p className="text-xs text-dark/50 dark:text-white/50 mt-1">
              Solde actuel: ${currentSolde.toFixed(2)}
            </p>
          </div>
